...
func setupTestServer(t *testing.T) (*httptest.Server, *Service, *mock.MockTask) {
	...
	// GET /rest/v0/srs/{id}/tasks - Get tasks for an SR
	mux.HandleFunc("GET /rest/v0/srs/{id}/tasks", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
func TestGetTasks(t *testing.T) {
	t.Run("passes limit and filter parameters", func(t *testing.T) {
		...
		srID := uuid.Must(uuid.FromString(testSRID1))
		tasks, err := service.GetTasks(context.Background(), srID, limit, filter)
	...
	t.Run("does not send limit param when zero", func(t *testing.T) {
		...
		srID := uuid.Must(uuid.FromString(testSRID1))
		_, err := service.GetTasks(context.Background(), srID, 0, "")
		assert.NoError(t, err)
	})
	...
	t.Run("returns error on http error", func(t *testing.T) {
		...
		srID := uuid.Must(uuid.FromString(testSRID1))
		tasks, err := service.GetTasks(context.Background(), srID, 0, "")

		assert.Error(t, err)
		assert.Nil(t, tasks)
	})

	t.Run("successfully retrieves tasks for an SR", func(t *testing.T) {
		server, service, _ := setupTestServer(t)
		defer server.Close()
...
func TestAddTag(t *testing.T) {
	...
	t.Run("successfully adds a tag to an SR", func(t *testing.T) {
		server, svc, _ := setupTestServer(t)
		defer server.Close()

		err := svc.AddTag(t.Context(), srID, "production")

		assert.NoError(t, err)
	})

	t.Run("returns error when tag is empty", func(t *testing.T) {
		server, svc, _ := setupTestServer(t)
		defer server.Close()

		err := svc.AddTag(t.Context(), srID, "")
...
func TestRemoveTag(t *testing.T) {
	...
	t.Run("successfully removes a tag from an SR", func(t *testing.T) {
		server, svc, _ := setupTestServer(t)
		defer server.Close()

		err := svc.RemoveTag(t.Context(), srID, "production")
	...
	t.Run("returns error when tag is empty", func(t *testing.T) {
		...
		err := svc.RemoveTag(t.Context(), srID, "")
